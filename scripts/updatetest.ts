...
try {
  ...
  globalThis.fetch = async () => reply([
    ...
    completeRelease('v2.7.0-rc.1', true),
    completeRelease('v2.6.0'),
  ]);
  const manager = new UpdateManager();
  const state = await manager.state(true);
  check('выбран самый новый стабильный релиз независимо от порядка API', state.tag === 'v2.6.0', state.tag);
  ...
  check('в запросе закреплён проверенный тег', request['tag'] === 'v2.6.0' && request['current'] === version, request);
  ...
  globalThis.fetch = async () => reply([{
    ...completeRelease('v2.6.0'), assets: [asset('ai-support.tar.gz')],
  }]);
  ...
  globalThis.fetch = async () => reply([completeRelease('v2.6.0')]);
  ...
  check('краткий сетевой сбой сохраняет последний проверенный релиз',
    afterFailure.tag === 'v2.6.0' && afterFailure.available && /temporary network/u.test(afterFailure.error ?? ''), afterFailure);
