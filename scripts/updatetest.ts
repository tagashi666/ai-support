...
try {
  ...
  globalThis.fetch = async () => reply([
    ...
    completeRelease('v2.4.0-rc.1', true),
    completeRelease('v2.3.0'),
  ]);
  const manager = new UpdateManager();
  const state = await manager.state(true);
  check('выбран самый новый стабильный релиз независимо от порядка API', state.tag === 'v2.3.0', state.tag);
  ...
  check('в запросе закреплён проверенный тег', request['tag'] === 'v2.3.0' && request['current'] === version, request);
